...
export function useGetAllAdminUsers() {
  ...
  return useQuery<AdminUser[]>({
    ...
    queryFn: async () => {
      ...
      console.log('[useGetAllAdminUsers] Fetching all admin users from backend');
      const users = await actor.getAllAdminUsers();
      console.log('[useGetAllAdminUsers] Fetched users:', users.length);
      return users;
    },
...
export function useCreateAdminUser() {
  ...
  return useMutation({
    mutationFn: async (user: { name: string; email: string; role: UserRole }) => {
      if (!actor) throw new Error('Actor not available');
      console.log('[useCreateAdminUser] Creating admin user:', user.name);
      
      // Call the refactored backend createAdminUser method
      // Backend now uses msg.caller as principal automatically
      const createdUser = await actor.createAdminUser(
        user.name,
        user.email,
        user.role
      );
      
      console.log('[useCreateAdminUser] User created successfully:', createdUser);
      return createdUser;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['adminUsers'] });
      toast.success('Admin user berhasil dibuat');
      console.log('[useCreateAdminUser] Success - invalidating adminUsers query');
    },
    onError: (error: any) => {
      ...
      if (isNetworkError(error)) {
        ...
        toast.error('Admin user dengan principal ini sudah ada');
      } else {
...
export function useUpdateAdminUser() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      console.log('[useUpdateAdminUser] Success - invalidating adminUsers query');
    },
    onError: (error: any) => {
...
export function useDeleteAdminUser() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      console.log('[useDeleteAdminUser] Success - invalidating adminUsers query');
    },
    onError: (error: any) => {
